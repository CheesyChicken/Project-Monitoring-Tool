...
router.get('/addformweek/:gi', requireAuth, (req, res, next) => {
    var gi = req.params.gi;
    var sql = "Select * from project_group where Group_id=?"
    connection.query(sql, [gi], (err, task, fields) => {
        if (err) {
            ...
            task.forEach(element => {
                ...
                enddate.setDate(enddate.getDate() - 7);
                var sql1 = "Update project_group set endDate = ? where Group_id = ?"
                connection.query(sql1, [enddate, gi], (err, task) => {
                    if (err) {
                        console.log(err);
                    }
                });
...
router.get('/week/:gi', requireAuth, (req, res) => {
    var gi = req.params.gi;
...
router.post('/week/:gi', requireAuth, (req, res, next) => {
    var gi = req.params.gi;
...
router.put('/week/:gi', requireAuth, (req, res, next) => {
