...
router.get('/ftrmaster/:clg/:dep', requireAuth, (req, res, next) => {
...
router.get('/ftrproject/:clg/:dep/:fm', requireAuth, (req, res, next) => {
    var gi = req.params.fm;
    var clg = req.params.clg;
    var dep = req.params.dep;
    var sql = "Select fp.*,p.FullName,g.Group_Name , g.Group_title from ftr_project_group fp Join persons p on p.Person_id=fp.reviewer_id Join project_group g on g.Group_id=fp.group_id  where fp.ftr_master_id = ?"
    connection.query(sql, [gi], (err, project, fields) => {
        if (err) {
            ...
            {
                p.Submit_Date = moment(p.Submit_Date).format("DD/MM/YYYY");
                p.Approved_date = moment(p.Approved_date).format("DD/MM/YYYY");
            ...
            connection.query(sql, [clg, dep], (err, group, fields) => {
                if (err) {
                    ...
                    count[1] = Number(project.length);
                    count[2] = Number(group[0].total) - Number(project.length)
                    ppc[0] = count;
                    ppc[1] = project;
                    res.json(ppc)
                }
...
router.post('/ftrmaster', requireAuth, (req, res, next) => {
...
router.post('/ftrmasters', requireAuth, (req, res, next) => {
...
router.post('/ftremail', requireAuth, (req,res,next)=>{
  var dt = req.body.dep;
