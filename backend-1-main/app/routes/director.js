...
router.get('/tabledir/:clg', requireAuth, (req, res, next) => {
...
router.get('/hoddir/:dep', requireAuth, (req, res, next) => {
...
router.get('/hoddirgroups/:dep', requireAuth, (req, res, next) => {
...
router.get('/homedir/:clg', requireAuth, (req, res, next) => {
    var cllg = req.params.clg;
...
router.get('/hometabledir/:clg', requireAuth, (req, res, next) => {
    var cllg = req.params.clg;
    var datas=[];
    var errors=[];
    var h=0;
    ...
    connection.query(sql, [cllg], (err, director, fields) => {
        if (err) {
          ...
          for(let i=0,j=1;i<director.length;i++){
            var sql = "select d.Department_id,d.Department_Name,(select count(*) from project_group g1 where g1.Status_id=1 and g1.Project_Type_id='2' and g1.Department_id=?)as lagind,(select count(*) from project_group g2 where g2.Status_id=1 and g2.Project_Type_id='1' and g2.Department_id=?)as laginh,(select count(*) from project_group g3 where g3.Status_id=2 and g3.Project_Type_id='2' and g3.Department_id=?)as ontind,(select count(*) from project_group g4 where g4.Status_id=2 and g4.Project_Type_id='1' and g4.Department_id=?)as ontinh,(select count(*) from project_group g5 where g5.Status_id=3 and g5.Project_Type_id='2' and g5.Department_id=?)as ledind,(select count(*) from project_group g6 where g6.Status_id=3 and g6.Project_Type_id='1' and g6.Department_id=?)as ledinh,(select count(*) from project_group g7 where g7.Department_id=?)as totaldep from department d where d.Department_id=? "
...
router.get('/alltabledir/:clg/:sts', requireAuth, (req, res, next) => {
    var cllg = req.params.clg;
...
router.get('/getstud/:clg', requireAuth, async(req,res)=>{
    var cllg = req.params.clg;
...
router.get('/yearstable/:cls/:yer', requireAuth, async(req,res)=>{
...
router.get('/yearstableall/:cls/', requireAuth, async(req,res)=>{
    var cllg = req.params.cls;
...
router.get('/yearwisestudent/:clg', async(req,res)=>{
    var clg = req.params.clg;

    var Query = `Select year,count(*) as cnt from project_group pg JOIN learner_project_reg lp ON pg.Group_id = lp.Group_id where pg.College_id = ? GROUP BY Year`;
    try {
        var count1 = await query(Query,[clg]);
        var obj = {};
...
router.get('/yearstudents/:clg/:year', async(req,res)=>{
    var clg = req.params.clg;
