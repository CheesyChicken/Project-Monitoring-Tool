...
router.get('/task_status/:groupid', requireAuth, (req, res, next) => {
    let groupid = req.params.groupid;
    var sql = "Select * from project_task where Group_id = ?"
    var error = [];
    var flag = 0;
    connection.query(sql,[groupid], (err, task, fields) => {
        if (err) {
            error[0] = err;
            console.log(err);
        }
...
router.post('/task_status', requireAuth, (req, res, next) => {
    ...
    connection.query(sql, [SN], (err, task) => {
        ...
        var dtt = moment(dt).format("YYYYMMDD");
        var at = new Date(task[0].Submitted_Date);
